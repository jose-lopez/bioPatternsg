...
patron(P, R, O, S) :-
	...
	final(NN, R, O, Final), 
        % base(C), length(C, L), 
	eventos_intermedios(O, S, N, Intermedios, NN),
...
eventos_intermedios(O, _, N1, [event(N1, Rel1, N)], N) :- 
	base(B), 
	member(event(N1, Rel1, N), B), % 1. Solo pueden haber eventos de la BC en los pathways. 
	member(Rel1, [interact,associate,recruit,activate]),
	...
	not(member(N,O)), not(member(N1,O)), not(ligand(N)), not(ligand(N1)). 
...
eventos_intermedios(O, L, N1, [event(N1, Rel1, N)|R], NL) :- L > 0, 
	base(B), 
	member(event(N1, Rel1, N), B), % 1. Solo pueden haber eventos de la BC en los pathways. 
	member(Rel1, [interact,associate,recruit,activate]),
%	member(event(N, Rel2, N2), B), % 1. Solo pueden haber eventos de la BC en los pathways. 
%	member(Rel2,[interact,associate,recruit,activate]),
	...
	not(member(N,O)), not(member(N1,O)), not(ligand(N)), not(ligand(N1)),
        LL is L - 1,
        eventos_intermedios(O, LL, N, R, NL),
